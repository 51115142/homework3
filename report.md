...
這個作業的目標是將一個單元數多項式使用「循環連結串列」表示，并能對其進行加法、減法、乘法、數值測試等執行。
...
1. 本程式成功達成題目要求，能以循環連結串列的方式正確表示多項式，並支援完整的輸入、輸出、動態記憶體配置與多項式運算。
...
這個作業的開發主軸採用有頭節點的循環連結串列結構來管理項目資料。所有操作均由 Polynomial 類別管理，只需透過物件與運算子重載進行互動，可以大幅提升使用與維護上的便利性。
